 ...
 * 规范要求的 PRD 按钮。
 *
 * 使用 `position: fixed` 固定在视口右上角，确保在任何布局、TabBar、滚动场景下都可见。
 * `top`/`right` 通过外层 className 传入，便于父级微调。
 */
export default function PrdButton({
  ...
  return (
    <button
      ...
      className={cn(
        'fixed top-4 right-4 z-30 flex items-center space-x-1.5 border px-3 py-1.5 rounded-md text-sm font-medium transition-colors shadow-sm',
        isOpen
